...
system.afterEvents.scriptEventReceive.subscribe(async ({ id, message, sourceEntity }) => {
    ...
    if (id == "hivemind:save") {
        const pos1 = player.getDynamicProperty("saveLocation1");
        const pos2 = player.getDynamicProperty("saveLocation2");
    ...
    if (id == "hivemind:load") {
        ...
        if (buildReq.status == ServerStatusResponse.Success) {
            ...
            const structure = buildReq.getData();
...
function getSelectionBounds(pos1, pos2, add = 0) {
    return {
...
function drawSelection(dimension, pos1, pos2) {
...
function runSaveStructure(dimension, pos1, pos2, onDone) {
    system.runJob(saveStructure(dimension, pos1, pos2, onDone))
}

function* saveStructure(dimension, pos1, pos2, onDone) {
...
function runPlaceStructure(structure, dimension, location) {
    system.runJob(placeStructure(structure, dimension, location))
}

function* placeStructure(structure, dimension, location) {
